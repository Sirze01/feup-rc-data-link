...
void assemble_data_packet(unsigned char *out_packet, int seq_no,
...
void assemble_control_packet(unsigned char *out_packet, int end, int no_args,
                             int arg1_size, unsigned char *arg1, ...) {
    ...
    for (int i = 1; i < no_args; i++) {
        ...
        unsigned char *data = va_arg(vlist, unsigned char *);
